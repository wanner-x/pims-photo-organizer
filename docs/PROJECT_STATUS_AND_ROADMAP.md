...
- 后台整理任务正在运行：`scripts\run_full_detection.ps1 -ExecuteConfirmedBatches`
...
- 现阶段 R18 标签链路已打通，但真正“看图判断性器官暴露”还需要接入视觉模型或本地 NSFW 模型。
...
R18 标签字段和审核展示已实现，但还没有真正接入图片视觉模型。
...
- 拆分静态文件。
- 增加系列建议筛选：待 AI、待确认、R18、低置信度、目标冲突。
- 增加隔离区管理页。
- 增加操作回滚页。
...
```powershell
pims run-safe-workflow --keep-root "\\192.168.31.10\personal_folder\网络写真集" --md5-limit 20000 --phash-limit 5000 --thumbnail-limit 2000 --min-series-assets 2
```

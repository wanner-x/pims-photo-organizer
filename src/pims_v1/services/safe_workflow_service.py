...
from pims_v1.services.visual_moderation_service import VisualModerationClient, build_visual_moderation_client
...
def _empty_ai_suggest_summary() -> dict[str, int]:
    return {
        "considered": 0,
        "processed": 0,
        "suggested": 0,
...
def _empty_r18_scan_summary() -> dict[str, int]:
    return {
        "considered": 0,
        "processed": 0,
        "flagged": 0,
...
def run_safe_workflow(
    *,
    ...
    moderation_client: VisualModerationClient | None = None,
    ...
    if keep_root and ai_suggest_limit > 0 and archive_client is not None:
        ai_suggest = suggest_series_organization_candidates(
            ...
            limit=ai_suggest_limit,
        )
    r18_scan = _empty_r18_scan_summary()
    if r18_scan_limit > 0:
        provider = moderation_client or build_visual_moderation_client(settings.r18_provider)
        r18_scan = review_series_r18_candidates(
            session=session,
            provider=provider,
            mode="workflow",
            limit=r18_scan_limit,
